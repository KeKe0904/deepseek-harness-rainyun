...
| 公网访问地址 | `DSH_TRUSTED_HOSTS` | 文本 | 空 | 否 | **重要**：填部署完成后浏览器地址栏里的**裸地址** `域名` 或 `IP:端口`（如 `38.246.252.220:9990`）。**不要带 `http://`、不要带路径、不要带结尾斜杠**（`0.1.0-rc.6-2` 起入口脚本会自动清洗格式，填完整 URL 也能用）。多个用空格分隔。DSH 的 `/api` 有浏览器信任围栏，未声明的 Host 会 403 |
